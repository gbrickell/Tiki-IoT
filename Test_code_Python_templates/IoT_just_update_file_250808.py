...
# decide if the file metadata should be used
metadata = "no"  # setting it to 'no' truncates the API response text to exclude everything from 'syntax' onwards in the response and makes decoding the response text a lot easier
b_metadata = metadata.encode('utf-8')
...
response = str( pi_iot_control_YYMMDD.gallery_fileupdate(ctypes.c_int(debug), ctypes.c_char_p(b_domain), ctypes.c_char_p(b_access_token), ctypes.c_char_p(b_fileId), ctypes.c_char_p(b_filepath), ctypes.c_char_p(b_filename), ctypes.c_char_p(b_filetitle), ctypes.c_char_p(b_filedescblank), ctypes.c_char_p(b_metadata) ) )

print ("\n*** gallery_fileupdate response:\n" )
response = response[2:-1]
print ( response ) 
if "update was not successful" in response :
    print("\n file update curl request failed or response was an empty \n")
elif "empty response from the curl request" in response:
    print("\n there was an empty response from the curl request \n")
else:
